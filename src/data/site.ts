...
type Copy = {
  ...
  hero: { eyebrow: string; title: string; lead: string; download: string; roadmap: string };
  features: { title: string; lead: string; items: Array<{ title: string; body: string }> };
  product: { title: string; lead: string; capture: string; editor: string; privacy: string; captureTools: string[]; editorTools: string[] };
  support: { title: string; lead: string; action: string; note: string };
...
export const copy: Record<Locale, Copy> = {
  de: {
    ...
    hero: { eyebrow: 'Chrome-Erweiterung', title: 'Erfassen. Erklären. Teilen.', lead: 'Erstelle Screenshots und Bildschirmaufnahmen direkt in Chrome. Bearbeite Details mit einem leistungsstarken, lokalen Editor.', download: 'Im Chrome Web Store installieren', roadmap: 'Entwicklung verfolgen' },
    ...
    support: { title: 'Hilfe, Fehler und Ideen', lead: 'Beschreibe Fehler mit nachvollziehbaren Schritten. Bitte veröffentliche keine Zugangsdaten, vertraulichen Aufnahmen oder personenbezogenen Daten.', action: 'Support-Anfrage erstellen', note: 'Kundenmeldungen werden zuerst geprüft; nur freigegebene Entwicklungstickets erscheinen im öffentlichen Tracker.' },
  ...
  en: {
    ...
    hero: { eyebrow: 'Chrome extension', title: 'Capture. Explain. Share.', lead: 'Create screenshots and screen recordings directly in Chrome. Edit every detail with a powerful local editor.', download: 'Install from the Chrome Web Store', roadmap: 'Follow development' },
    ...
    product: { title: 'Capture without detours', lead: 'Choose exactly what to capture, then turn the result into a clear explanation without changing tools.', capture: 'Flexible capture modes', editor: 'Built-in professional editor', privacy: 'No telemetry for your content', captureTools: ['Visible area', 'Full page', 'Element', 'Window', 'Recording'], editorTools: ['Arrow', 'Text', 'Blur', 'Highlight', 'Export'] },
    support: { title: 'Help, bugs, and ideas', lead: 'Describe issues with reproducible steps. Please do not publish credentials, confidential captures, or personal data.', action: 'Create a support request', note: 'Customer reports are reviewed first; only approved engineering tickets appear in the public tracker.' },
